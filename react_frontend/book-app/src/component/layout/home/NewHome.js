...
import { LinearProgress, Container, Grid, Paper ,Tooltip} from "@material-ui/core";
...
export default function NewHome() {
  ...
  const columns = [
    { field: "_id", title: "ID", editable: false, hidden: true, sorting :false },
    { field: "title", title: "Book Name", editable: false , sorting :false },
    { field: "author", title: "Author" , sorting :false},
    { field: "date_added", title: "Date Added", editable: false, hidden: true ,defaultSort:"desc" },
  ];
  useEffect(() => {
    setData(state);
  }, [state]);
          ...
          <Grid item xs>
            {/* <Paper elevation={5} className={classes.paper}> */}
              <MaterialTable
                ...
                columns={columns}
                editable={{
                  isEditable: (rowData) => store.user.USER_CURRENT_STATUS === true,
                  // isDeletable: (rowData) =>
                  ...
                  onRowDelete: (selectedRow) =>
                    new Promise((resolve, reject) => {
                      setTimeout(() => {
                        ...
                        dispatch(delete_book({ id: index , user : store.user.USER_ID, }));
                        resolve();
                      }, 1000);
                    }),
                }}
